...
const TuckShopScreen = () => {
  ...
  const [qty,setQty]=useState(1);
   const showModal=(item)=>{
    ...
    const orderItem = async (item, qty, image, totalPrice) => {
      try {
        const storedTuckShopCart = await AsyncStorage.getItem('tuckShopCart');
        let updatedTuckShopCart = [];
        if (storedTuckShopCart) {
          updatedTuckShopCart = JSON.parse(storedTuckShopCart);
        }
        const newItem = {
          item,
          qty,
        ...
        updatedTuckShopCart.push(newItem);
        await AsyncStorage.setItem('tuckShopCart', JSON.stringify(updatedTuckShopCart));
      } catch (error) {
      }
  ...
  <View className="rounded-full " style={{  marginRight:10 }}>
    ...
    {item.qty}
         ...
         <Button title="Reserve" color="#BF1A2F" className="rounded-lg" onPress={() => showModal(item)} />
         </View>
        </View>
        ))}
      </View>
  ...
  <Text style={{ textAlign: "center", width:"70%" , marginLeft:90, color:fontColor}} className="text-lg font-bold self-center"><Text style={{color:"gray"}}>Balance:₹</Text>{1000-qty*selectedItem.price}</Text>
  <View style={{ flexDirection: "row", justifyContent: "space-evenly" }}>
    <TouchableOpacity onPress={()=>orderItem(selectedItem.item,qty,selectedItem.image,qty*selectedItem.qty)}
    ...
    <TouchableOpacity onPress={()=>orderItem(selectedItem.item,qty,selectedItem.image,qty*selectedItem.price)}
      className="rounded-lg self-center"
      style={{ marginTop: 40, backgroundColor: "#BF1A2F", width: "40%" }}>
