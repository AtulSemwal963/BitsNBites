...
export default function TuckShopCart() {
  ...
  //       if (storedTuckShopCart !== null) {
  //         const parsedTuckShopCart = JSON.parse(storedTuckShopCart);
  //         setTuckShopCart(parsedTuckShopCart);
  //       } 
  //     } catch (error) {
  //       console.error('Error loading TuckShop cart:', error);
  //     }
  //   };
  ...
  //   try {
  //     const updatedCart = [...tuckShopCart];
  //     updatedCart.splice(index, 1);
  //     setTuckShopCart(updatedCart);
  //     await AsyncStorage.setItem('tuckShopCart', JSON.stringify(updatedCart));
  //   } catch (error) {
  ...
  // const totalSum = tuckShopCart.reduce((total, item) => total + item.price, 0);
  // useEffect(() => {
  ...
  const placeOrder = async () => {
    try {
      const storedCart = await AsyncStorage.getItem('tuckShopCart');
      let cartItems = [];
      if (storedCart) {
      ...
      const response = await fetch("https://bits-n-bitesbackendserver.onrender.com/placeOrderTuckShop", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
      ...
      if (response.ok) {
       ...
       await deductBalance();
       getBalance();
       await AsyncStorage.removeItem('tuckShopCart');
